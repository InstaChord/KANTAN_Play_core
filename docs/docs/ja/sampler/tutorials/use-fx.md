...
FXモードでは、再生中のビートやLoopへリアルタイムに効果を加えられます。まずは分かりやすいRepeatとFilterから試します。

FX操作そのものはRecへ記録されません。演奏全体をPerformance Recordingした場合は、FXを含む最終出力がWAVへ残ります。

## 1. Loopを再生する

1. ビートまたはRecしたLoopを用意します
2. `Fn1`を押して再生します
...
- P9 `BEAT`：ビートへかける
- P10 `PARTS`：サンプラー、Bass、Melody、Chordへかける
...
まずP2を短く押し、次にP3、P4と細かくしてみます。パッドを離すと通常再生へ戻ります。
...
Filter、Gater、Crusher、Delayは、パッドを押している間にダイヤル、画面の上下位置、または本体の傾きで値を変えられます。最初はダイヤルだけを使うと操作が分かりやすくなります。
...
同時に複数のFX パッドを押した場合は、あとから押したFXが有効になります。

## Mixerを開く

FXモードで`Fn3`を押すとMixerへ切り替わります。もう一度`Fn3`を押すまでMixer表示を維持します。

- Part パッドを短く押す：そのパートのRecをMute / Unmute
- Part パッドを押したまま下のダイヤルを回す：パート音量を変更
- P9〜P12：Mix 1〜4の保存・呼び出し

<!-- IMAGE MIX-01: Mixerの6パートフェーダーとP9〜P12のMix Slotが分かる画面。 -->

MixerのMuteは生演奏を止めません。Recした演奏を消音しながら、同じパッドを手で演奏できます。
