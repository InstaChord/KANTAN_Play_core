...
2. 確認中は画面タップまたはOK再押下で確定し、その他のボタンでキャンセルできること
3. 確定後に`SAFE TO REMOVE`となり、SD上の一覧を再度開けないこと
4. 同じカードと別カードをそれぞれ挿し、`Load SD Card`で`READY`へ戻ること
5. カードなし起動で`NOT INSERTED`となり、後挿し後のLoadで認識すること
6. 未対応または壊れたカードで`READY`にせず、失敗表示が一度だけ出ること
7. Eject / Loadを10回繰り返し、古い一覧、Musicハンドル、プレビューが残らないこと
