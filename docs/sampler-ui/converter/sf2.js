...
function fail(message){throw new Error(`SF2を解析できません: ${message}`);}
...
function chunks(dv,start,length){const out=[];let p=start,end=start+length;while(p+8<=end){const id=fourCC(dv,p),size=dv.getUint32(p+4,true),offset=p+8;if(offset+size>end)fail(`${id}チャンクがファイル範囲を超えています`);out.push({id,offset,size});p=offset+size+(size&1);}return out;}
function list(dv,top,type){const found=top.find(c=>c.id==='LIST'&&c.size>=4&&fourCC(dv,c.offset)===type);if(!found)fail(`LIST ${type}がありません`);return chunks(dv,found.offset+4,found.size-4);}
function required(cs,id){const found=cs.find(c=>c.id===id);if(!found)fail(`${id}チャンクがありません`);return found;}
function records(dv,chunk,size,read){if(chunk.size%size)fail(`${chunk.id}チャンクのサイズが不正です`);const out=[];for(let p=chunk.offset;p<chunk.offset+chunk.size;p+=size)out.push(read(p));return out;}
...
function zones(headers,bags,gens,index,terminal){const first=headers[index].bagIndex,last=headers[Math.min(index+1,terminal)].bagIndex,out=[];for(let b=first;b<last;b++){if(!bags[b]||!bags[b+1])fail('bag indexが不正です');const entries=gens.slice(bags[b].genIndex,bags[b+1].genIndex);out.push({entries,byOp:new Map(entries.map(g=>[g.op,g]))});}return out;}
...
export function parseSf2(input){
  ...
  if(dv.byteLength<12||fourCC(dv,0)!=='RIFF'||fourCC(dv,8)!=='sfbk')fail('SoundFont 2（RIFF sfbk）形式ではありません。SF3は非対応です');
  const declared=dv.getUint32(4,true);if(declared+8>dv.byteLength)fail('RIFFサイズが不正です');
  ...
  const smpl=required(sdta,'smpl');if(smpl.size&1)fail('smplチャンクのサイズが不正です');
  const samples=new Int16Array(smpl.size/2);for(let i=0;i<samples.length;i++)samples[i]=dv.getInt16(smpl.offset+i*2,true);
  if(phdr.length<2||inst.length<2||shdr.length<2)fail('終端レコードがありません');
...
export function resolvePresetRegions(sf2,presetIndex,key=60,velocity=110){
  const preset=sf2.presets[presetIndex];if(!preset)throw new Error('音色を選んでください');const p=split(preset.zones,OP.instrument),out=[];
