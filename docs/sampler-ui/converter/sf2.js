...
const OP={start:0,end:1,startLoop:2,endLoop:3,startCoarse:4,endCoarse:12,delayVolEnv:33,attackVolEnv:34,holdVolEnv:35,decayVolEnv:36,sustainVolEnv:37,releaseVolEnv:38,instrument:41,keyRange:43,velRange:44,startLoopCoarse:45,keynum:46,velocity:47,attenuation:48,endLoopCoarse:50,coarseTune:51,fineTune:52,sampleID:53,sampleModes:54,rootKey:58};
const SUPPORTED=new Set([0,1,2,3,4,12,33,34,35,36,37,38,41,43,44,45,46,47,48,50,51,52,53,54,58]);
const UNSUPPORTED=new Map([[5,'modulation LFO'],[6,'vibrato LFO'],[7,'modulation envelope'],[8,'filter'],[9,'filter Q'],[10,'LFO → filter'],[11,'envelope → filter'],[13,'LFO → volume'],[15,'chorus'],[16,'reverb'],[17,'pan'],[21,'modulation LFO'],[22,'modulation LFO'],[23,'vibrato LFO'],[24,'vibrato LFO'],[25,'modulation envelope'],[26,'modulation envelope'],[27,'modulation envelope'],[28,'modulation envelope'],[29,'modulation envelope'],[30,'modulation envelope'],[56,'scale tuning'],[57,'exclusive class']]);
...
export function firmwareEnvelopeMs(ms,type){const ranges={delay:[0,6553.5],attack:[0,5000],hold:[0,5000],decay:[0,60000],release:[10,10000]},bounds=ranges[type]||ranges.attack,value=type==='delay'?Math.round(ms*10)/10:Math.round(ms);return clamp(value,bounds[0],bounds[1]);}
export function sustainCentibelsToQ15(centibels){return clamp(Math.round(32768*Math.pow(10,-Math.max(0,Number(centibels)||0)/200)),0,32768);}
...
export function resolvePresetRegions(sf2,presetIndex,key=60,velocity=110){
  ...
  for(const pz of matching(p.locals,key,velocity)){const instrumentIndex=pz.byOp.get(OP.instrument).raw,instrument=sf2.instruments[instrumentIndex];if(!instrument)continue;const izs=split(instrument.zones,OP.sampleID);
    for(const iz of matching(izs.locals,key,velocity)){const zs=[p.global,pz,izs.global,iz].filter(Boolean);let kr=[0,127],vr=[0,127];for(const z of zs){kr=intersect(kr,range(z,OP.keyRange));vr=intersect(vr,range(z,OP.velRange));}if(key<kr[0]||key>kr[1]||velocity<vr[0]||velocity>vr[1])continue;
      const sampleIndex=iz.byOp.get(OP.sampleID).raw,sample=sf2.sampleHeaders[sampleIndex];if(!sample)continue;const start=sample.start+sum(zs,OP.start)+sum(zs,OP.startCoarse)*32768,end=sample.end+sum(zs,OP.end)+sum(zs,OP.endCoarse)*32768,loopStart=sample.startLoop+sum(zs,OP.startLoop)+sum(zs,OP.startLoopCoarse)*32768,loopEnd=sample.endLoop+sum(zs,OP.endLoop)+sum(zs,OP.endLoopCoarse)*32768,modes=last(zs,OP.sampleModes)??0;
      const unsupported=[...new Set(zs.flatMap(z=>z.entries.filter(g=>!SUPPORTED.has(g.op)&&g.op!==60).map(g=>UNSUPPORTED.get(g.op)||`generator ${g.op}`)))];if(sf2.hasPresetModulators||sf2.hasInstrumentModulators)unsupported.push('modulators');if(sample.sampleType&0x8000)unsupported.push('ROM sample');
      ...
      const sustainPresent=zs.some(z=>z.byOp.has(OP.sustainVolEnv)),sustainLevelQ15=sustainPresent?sustainCentibelsToQ15(sum(zs,OP.sustainVolEnv)):32768,presetZoneIndex=preset.zones.indexOf(pz),instrumentZoneIndex=instrument.zones.indexOf(iz);
      out.push({id:`${presetIndex}:${presetZoneIndex}:${instrumentIndex}:${instrumentZoneIndex}:${sampleIndex}:${start}:${end}:${loopStart}:${loopEnd}`,presetName:preset.name,instrumentName:instrument.name,sampleName:sample.name,sampleIndex,keyRange:kr,velRange:vr,sampleRate:sample.sampleRate,rootNote:tuning.rootNote,previewNote:forcedKey!==undefined&&forcedKey<=127?forcedKey:key,tuneCents:tuning.tuneCents,start,end,loopStart,loopEnd,sustainMode:(modes&1)?'loop':'oneShot',delayMs:firmwareEnvelopeMs(timecents(zs,OP.delayVolEnv),'delay'),attackMs:firmwareEnvelopeMs(timecents(zs,OP.attackVolEnv),'attack'),holdMs:firmwareEnvelopeMs(timecents(zs,OP.holdVolEnv),'hold'),decayMs:firmwareEnvelopeMs(timecents(zs,OP.decayVolEnv),'decay'),sustainLevelQ15,releaseMs:firmwareEnvelopeMs(timecents(zs,OP.releaseVolEnv),'release'),initialAttenuationCb:Math.max(0,sum(zs,OP.attenuation)),sampleType:sample.sampleType,sampleLink:sample.sampleLink,unsupported});
    }
  }return out;
}
