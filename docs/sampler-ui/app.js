...
(() => {
  ...
  function sf2LayerCard(item,index){
    const {region,settings}=item,selected=selectedSf2Layers(),waveRegion=index===1&&settings.pcmMode==='same'?selected[0].region:region,source=index===1?el('select',{},el('option',{value:'same',selected:settings.pcmMode==='same'?'':null},'Same waveform as Layer 1'),el('option',{value:'own',selected:settings.pcmMode!=='same'?'':null},'Different waveform')):null;if(source)source.addEventListener('change',()=>{settings.pcmMode=source.value;invalidateSynth();renderSamples();});const card=el('section',{class:'synth-layer'},el('div',{class:'sf2-launcher'},el('h3',{},`Layer ${index+1}: ${region.sampleName||'Sound'}`),index===1?el('button',{onclick:()=>toggleSf2Region(region)},'Remove Layer 2'):null),index===1?sf2Field('Layer 2 Sound Source',source):null,el('div',{class:'actions'},el('button',{onclick:()=>previewSf2Layer(waveRegion,settings,region)},`Preview Layer ${index+1}`),el('button',{onclick:()=>sf2Player.stop(true)},'Stop')));
  ...
  function sf2Panel(){
    ...
    panel.append(el('h3',{},'Source Sounds'));if(sf2Editor.regions.length>1)panel.append(el('p',{class:'warning',role:'status'},sf2Editor.regions.length>2?'This preset contains more than two matching sounds. Choose up to two; additional layers are not included. Unsupported SoundFont features are ignored.':'Choose one or two sounds. The second selection becomes Layer 2. Unsupported SoundFont features are ignored.'));if(!sf2Editor.regions.length)panel.append(el('p',{class:'error'},'No sound matches the current root note and velocity.'));else panel.append(el('fieldset',{class:'sf2-candidates'},el('legend',{},'Select up to two sounds'),sf2Editor.regions.map(sf2Candidate)));
  ...
  function importedKtsLayerCard(settings,index){
    const remove=index===1?el('button',{onclick:()=>{sf2Editor.ktsLayers.splice(index,1);invalidateSynth();renderSamples();}},'Remove Layer 2'):null,source=index===1?el('select',{},el('option',{value:'same',selected:settings.pcmMode==='same'?'':null},'Same waveform as Layer 1'),el('option',{value:'own',selected:settings.pcmMode!=='same'?'':null,disabled:settings.independentPcm?null:''},'Different waveform')):null;if(source)source.addEventListener('change',()=>{settings.pcmMode=source.value;invalidateSynth();renderSamples();});const card=el('section',{class:'synth-layer'},el('div',{class:'sf2-launcher'},el('h3',{},`Layer ${index+1}`),remove),index===1?sf2Field('Layer 2 Sound Source',source,settings.independentPcm?'':'This file contains one shared waveform.'):null,el('div',{class:'actions'},el('button',{onclick:()=>previewImportedKtsLayer(settings)},`Preview Layer ${index+1}`),el('button',{onclick:()=>sf2Player.stop(true)},'Stop')));
